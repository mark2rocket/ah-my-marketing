...
```
/campaign-plan → /copy-lmf → /taxonomy → /ad-creative → /meta-setup → /ad-optimize → /ad-analysis
       │               │           │            │               │              │              │
...
모든 산출물은 `output/{클라이언트명}/` 하위에 **날짜 포함 폴더**로 저장한다.
...
| 트랙 | 순서 | 명령어 | 역할 | 독립 실행 |
|------|------|--------|------|----------|
| Paid | 1 | `/campaign-plan` | 캠페인 전략·KPI·스프린트 기획 | 가능 |
| Paid | 2 | `/copy-lmf` | LMF 4단계 소구점·카피 발굴 | 가능 |
| Paid | 3 | `/taxonomy` | 광고 네이밍 컨벤션 정의·검증 | 가능 |
| Paid | 4 | `/ad-creative` | 광고 소재 HTML → PNG 제작 | 가능 |
| Paid | 5 | `/meta-setup` | 캠페인→광고세트→광고 세팅 명세서 | 가능 |
| Paid | 6 | `/ad-optimize` | 집행 중 성과 최적화 (예산·소재·타겟) | 가능 |
| Paid | 7 | `/ad-analysis` | 캠페인 종료 후 결과 분석 + Compounding | 가능 |
| Organic | — | `/cardnews` | SNS 카드뉴스 기획·제작 (HTML→PNG) | 가능 |
| Organic | — | `/reels` | 릴스 기획·제작 (HTML→Playwright MP4) | 가능 |
| Organic | — | `/blog` | 블로그·뉴스레터 초안 작성 | 가능 |
| Meta | — | `/harness-review` | 하네스 자기 검토·개선 | 가능 |
