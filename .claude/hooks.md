...
```
체크:
□ docs/clients/{name}/brand-guide.md 존재하는가?
  → ✅ 존재: 통과 (Phase 0에서 제품 선택 진행)
  → ❌ 없음: 아래를 출력하고 중단.

  ❌ brand-guide.md가 없습니다. 브랜드·제품 정보를 먼저 등록해주세요.
  Step 1. /amm-brand-setup 실행 → 브랜드·제품 정보 등록
  Step 2. 원래 요청한 스킬 재시작
```

### GATE-1: 클라이언트 및 계정 컨텍스트 확인
**적용:** 모든 스킬
...
□ 광고 계정명이 명시되었는가?
□ docs/clients/{name}/accounts/{account}/ 폴더가 존재하는가?
  → 없으면: 아래 전체 경로를 출력 후 중단.

  ❌ 광고 계정 폴더가 없습니다. 아래 순서로 진행하세요:
  Step 1. docs/clients/{name}/accounts/{account}/ 폴더 생성
  Step 2. /amm-taxonomy 실행 → taxonomy.md 생성
  Step 3. 원래 요청한 스킬 재시작

  ※ 계정명 예시: main / kr-performance / brand-01 / default
```
...
```
체크:
□ docs/clients/{name}/accounts/{account}/taxonomy.md 존재하는가?
...
```
체크:
□ output/{name}/{account}/copy/lmf-brief-*.md 존재하는가?
...
`output/{name}/{account}/copy/copy-candidates-*.md` 파일의 물리적 저장 여부를 의미한다.
파일이 디스크에 저장되어야만 Generator 세션이 완료된 것으로 간주한다.

```
체크:
□ output/{name}/{account}/copy/copy-candidates-{YYMM}.md 파일이 저장되어 있는가?
  ...
  지금 해야 할 것:
  Step 1. 생성된 카피 후보를 output/{name}/{account}/copy/copy-candidates-{YYMM}.md로 저장
...
```
체크:
□ 산출물이 output/{name}/{account}/ 하위에 저장되었는가?
□ 파일명이 택소노미 컨벤션을 따르는가?
...
```
체크:
□ docs/clients/{name}/accounts/{account}/campaign-log.md에 이번 캠페인 결과가 기록되었는가?
□ docs/clients/{name}/accounts/{account}/lmf-learnings.md에 winner 소구점 + 고객 언어가 추가되었는가?
  → 미기록 시: 다음 스킬 실행 차단. 아래를 출력.

  ❌ Compounding 업데이트 없이 다음 단계로 진행할 수 없습니다.
  
  지금 해야 할 것:
  Step 1. docs/clients/{name}/accounts/{account}/campaign-log.md에 이번 결과 append
  Step 2. docs/clients/{name}/accounts/{account}/lmf-learnings.md에 winner 소구점 + 고객 언어 append
