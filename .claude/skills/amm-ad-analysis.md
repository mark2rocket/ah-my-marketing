...
3. "분석할 캠페인명을 알려주세요. (택소노미 기준 전체 이름, 예: ACQ_AC_MYSVC_S01_260419)"
4. "분석 기간은 언제부터 언제까지인가요?"
5. "성과 데이터를 어떻게 제공하실 건가요? (직접 입력 / 파일 첨부 / 캡처 이미지)"
6. "이번 캠페인의 Primary Metric은 무엇인가요? (CPI / CPA / CTR / ROAS 등)"
...
```
1. docs/clients/{name}/accounts/{account}/campaign-log.md 읽기 (이전 캠페인 패턴 파악)
2. docs/clients/{name}/accounts/{account}/lmf-learnings.md 읽기 (계정별 누적 인사이트)
3. docs/clients/{name}/accounts/{account}/taxonomy.md 읽기 (VP 코드 매핑)
4. output/{name}/{account}/copy/ice-priority-*.md 읽기 (기존 ICE 예측값)
```
...
**1. `docs/clients/{name}/accounts/{account}/campaign-log.md` 자동 업데이트:**
```markdown
### [{날짜}] {캠페인명}
Winner: VP{N} — {소구점}
Primary Metric: {결과}
...
**2. `docs/clients/{name}/accounts/{account}/lmf-learnings.md` 자동 업데이트:**
```markdown
## [{날짜}] {클라이언트명} — {캠페인명}
...
```
output/{name}/{account}/analysis/{YYMMDD}-analysis/
└── analysis.md           — 상세 분석 리포트
```

- `docs/clients/{name}/accounts/{account}/campaign-log.md` 업데이트 (append)
- `docs/clients/{name}/accounts/{account}/lmf-learnings.md` 업데이트 (append)
...
→ `docs/clients/{name}/accounts/{account}/campaign-log.md` (이전 캠페인 패턴)
→ `docs/clients/{name}/accounts/{account}/lmf-learnings.md` (계정별 누적 인사이트)
