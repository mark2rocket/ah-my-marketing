# /ad-creative — 메타 광고 소재 제작

## 트리거

`/ad-creative` 또는 "광고 소재", "배너 제작", "소재 만들기"

## 역할

`/copy-lmf`에서 발굴한 소구점·카피를 메타 광고 소재(이미지)로 제작한다.
...
- `/taxonomy` 완료 → VP 코드 정의됨
- `/copy-lmf` 완료 → `output/{name}/copy/copy-final-*.md` 존재
...
```
1. output/{name}/copy/copy-final-*.md 읽기
2. 제작할 VP 소구점 + 헤드라인 + 서브카피 + CTA 확인
3. docs/clients/{name}/brand-guide.md 읽기 (컬러, 폰트)
4. 택소노미 기준 파일명 생성
```
...
→ `.claude/skills/copy-lmf.md` (카피 원본)
